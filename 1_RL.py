...
def get_data():
    ...
    speed[i] = control_RL(ep_T_out[i], ep_HR_out[i], ep_Solar_direct[i], T_out_ini, ep_T[i], ep_HR[i], PMV[i], T_c[i],
                          HR_c[i], PMV_c[i], C_ini, F_ini, Elec_price, Elec_price1, t, t1)  # 可能需要float，[i,0]是因为要维度相同
    ...
    '''
    # 创建需要导出到Excel的参数字典
    dic = {'cooling_sensible': cooling_load[:, 0], 'ep_Qs_zone': ep_Qs_zone[:, 0],
           'ep_Qs_supplyair': ep_Qs_supplyair[:, 0], }  # [:,0]的意思是取第一列的所有行，把二维ndarray变成一维array，字典不能把键值对中的值value设成二维
